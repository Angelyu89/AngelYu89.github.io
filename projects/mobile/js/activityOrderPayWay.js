...
$(document).ready(function(){
	...
	$(".mettingOrderpayWayBtn").on("tap", function() {
			...
			if(payWay == 1) {                                                   
				window.location.href = box_webapp_url+"/pay/payment?orderNo="+getQueryString('orderNo')+'&paychannel=1'
			} else if(payWay == 3){
				$.ajax({
					type: "get",
					url:box_webapp_url+"/pay/payment?orderNo="+getQueryString('orderNo')+'&paychannel=3',
		...
		function getQueryString(orderNo) {
			//以id解析当前的url(如：http://127.0.0.1:8020/mobile/activity-order-xq.html?id=%2014)
			var reg = new RegExp('(^|&)' + orderNo + '=([^&]*)(&|$)', 'i');
			var r = window.location.search.substr(1).match(reg);
			//      console.log(r);
			if(r != null) {
				return decodeURI(r[2]); //获取ClassName的参数
